...
BattleManager.invokeCounterAttack = function(subject, target) {
    ...
    action.setSkill(COUNTER_SKILL_ID);//action.setAttack();
...
BattleManager.invokeMagicReflection = function(subject, target) {
    ...
    action.setSkill(COUNTER_MAGIC_ID);//action.setAttack();
    action.apply(subject);
    this._logWindow.displayActionResults(target, subject);
	if (subject.isDead()) subject.performCollapse();
};
...
Window_Help.prototype.drawBattler = function(battler) {
    ...
    this.drawText(text, wx, wy, 256, 'center');
	this.drawActorHp(battler,wx+256,wy,192);
	this.drawActorMp(battler,wx+480,wy,192);
};
