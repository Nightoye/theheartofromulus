...
Scene_Equip.prototype.create = function() {
	...
	//this._helpWindow.width = Graphics.boxWidth - Moghunter.scEquip_HelpWindowX;
	...
	this._itemWindow.width = Graphics.boxWidth-Moghunter.scEquip_ItemWindowX;
	this._itemWindow.height = Graphics.boxHeight-Moghunter.scEquip_ItemWindowY-96;
...
Scene_Equip.prototype.createCommandWindow = function() {
    ...
    var ww = Graphics.boxWidth - Moghunter.scEquip_ComWindowX;
...
Scene_Equip.prototype.createSlotWindow = function() {
    ...
    var ww = Graphics.boxWidth - Moghunter.scEquip_SlotWindowX;
...
Window_EquipStatus.prototype.createFaceSprite = function() {
	...
	this._faceSprite.x = 300;
	this._faceSprite.y = -this.y;
...
Window_EquipStatus.prototype.refreshFaceSprite = function() {
	 this._faceSprite.bitmap = ImageManager.loadMenusFaces3("Actor_" + this._actor._actorId);
};
...
Window_EquipStatus.prototype.windowHeight = function() {
    return Graphics.boxHeight-96-Moghunter.scEquip_StatusWindowY;
};
...
Window_EquipStatus.prototype.refreshXparam = function(){
	var place = 0;
	const y = this.lineHeight() * 10;
    if (this._tempActor) {
		this.drawHorzLine(y-this.lineHeight()-8);
		const xparamText = ['命中','回避','暴击','暴击回避','魔法回避','魔法反射','反击'];
		for(var i = 0; i < 7; i++) {
			var diffvalue = this._tempActor.xparam(i) - this._actor.xparam(i);
			if(diffvalue !== 0) {if (diffvalue > 0) {var sx = this._parData[0];}
			else if (diffvalue < 0) {var sx = this._parData[0] * 2;}
			else {var sx = 0};
				this.contents.blt(this._parImg, sx, 0, this._parData[0], this._parData[1], 188, place*32+y-8);
				var value = Math.floor(this._actor.xparam(i)*100);
				var newValue = Math.floor(this._tempActor.xparam(i)*100); 
				this.changeTextColor(this.systemColor());
				this.drawText(xparamText[i],6,place*32+y-16, 120, 'left');
				this.resetTextColor();
				this.drawText(value+"%", 120, place*32+y-16, 60, 'right');
				this.changeTextColor(this.paramchangeTextColor(diffvalue));
				this.drawText(newValue+"%", 202, place*32+y-16, 60, 'right');
				this.resetTextColor();
				place++;
			}
		}
